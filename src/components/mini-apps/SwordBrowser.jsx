...
import { generateWeapon, WEAPON_TYPES } from "./weaponGenerator";
...
function StatPill({ label, value }) {
    ...
    return (
        <span style={{
            ...
            background: positive ? "rgba(0,0,0,0.06)" : "rgba(200,0,0,0.08)",
...
function PartRow({ slot, part }) {
    ...
    return (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
            <div>
                <span style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", opacity: 0.4, marginRight: 8 }}>{slot}</span>
                <span style={{ fontSize: 14, fontWeight: 500 }}>{part.name}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 12, color: originColor, opacity: part.origin ? 0.6 : 0.35 }}>{part.origin ?? "Neutral"}</span>
                <span style={{
...
export default function SwordBrowser() {
    ...
    return (
        <div style={{ maxWidth: 480, margin: "2rem auto", fontFamily: "system-ui, sans-serif", color: "#1a1a1a" }}>
                ...
                {WEAPON_TYPES.map(({ id, label }) => (
                    <button
                        ...
                        style={{
                            ...
                            border: selectedType === id ? "2px solid #1a1a1a" : "2px solid #e5e7eb",
                            background: selectedType === id ? "#1a1a1a" : "#fff",
            ...
            {/* Main card */}
            <div style={{ border: `2px solid ${rarityColor}`, borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
                {/* Header */}
                <div style={{ background: rarityColor, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                        <div style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "rgba(255,255,255,0.6)", marginBottom: 2 }}>
                            {weapon.typeLabel}
                        </div>
                ...
                <div style={{ padding: "16px 18px", background: "#fff" }}>
                    ...
                    <div>
                        {weapon.slots.map(({ key, label }) => (
                            <PartRow key={key} slot={label} part={weapon.parts[key]} />
                        ))}
                    </div>
                </div>
            </div>
            ...
            {history.length > 0 && (
                <div style={{ marginTop: 20 }}>
                    <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", opacity: 0.4, marginBottom: 8 }}>Recent</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {history.map((w, i) => (
                            <div
                                key={i}
                                onClick={() => { setHistory(h => [weapon, ...h.filter((_, j) => j !== i)].slice(0, 8)); setWeapon(w); setSelectedType(w.type); }}
                                style={{
                            ...
                            >
                                <div>
                                    ...
                                    <span style={{ fontSize: 11, opacity: 0.4, marginLeft: 6 }}>{w.typeLabel}</span>
                                </div>
                                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                    <span style={{ fontSize: 12, opacity: 0.5 }}>{w.origin}</span>
                                    <span style={{ fontSize: 12, fontWeight: 600, color: RARITY_COLORS[w.rarity] }}>{w.rarity}</span>
                                </div>
                            </div>
                        ))}
                    </div>
