        ...
        <div style={{ viewTransitionName: "page-content" }}>
            ...
            {adminMode && isAdmin && (
                <div className="flex w-full bg-(--accent) border border-(--outline-brown)/50 md:rounded-t mt-4 mb-4">
                    ...
                    <button onClick={async () => { await addBlock({ nextOrder: 0, type: "title" }); }} className="flex-1 py-2 text-sm text-(--text-color) hover:bg-(--surface-background) cursor-pointer">
                        + Title Block
                    </button>
                </div>
            )}
                ...
                .map((block) => {
                    ...
                    const buttons = adminMode && isAdmin ? (
                        <div className="flex w-full bg-(--accent) border border-t-0 border-(--outline-brown)/50 md:rounded-b mb-4">
                            ...
                            <button onClick={async () => { await addBlock({ nextOrder: block.order + 1, type: "title" }); }} className="flex-1 py-2 text-sm text-(--text-color) hover:bg-(--surface-background) cursor-pointer">
                                + Title Block
                            </button>
                        </div>
                    ) : null;
